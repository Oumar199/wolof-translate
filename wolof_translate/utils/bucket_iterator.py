...
class SequenceLengthBatchSampler(Sampler):
    ...
    def calculate_length(self):
        ...
        sorted_indices = sorted(
            self.data_info.keys(), key=lambda i: self.data_info[i]["length"]
        )

        # Group indices into batches of sequences with the same length
        for boundary in self.boundaries:
            batch = [
                i for i in sorted_indices if self.data_info[i]["length"] <= boundary
            ]  # Filter indices based on length boundary
            self.batches.append(batch)
            sorted_indices = [
                i for i in sorted_indices if i not in batch
        ...
        self.length = sum(
            ceil(len(batch) / batch_size)
            for batch, batch_size in zip(self.batches, self.batch_sizes)
            if len(batch) % batch_size != 1 or not self.drop_unique
        )
    ...
    def __iter__(self):
        ...
        for batch_indices, batch_size in zip(self.batches, self.batch_sizes):
            ...
            if remaining_indices > 0 and remaining_indices != 1 or not self.drop_unique:
                ...
                # Yield the current bucket
                yield [
                    self.data_info[i]["index"]
                    for i in batch_indices[-remaining_indices:]
                ]
...
class BucketSampler(Sampler):
    def __init__(
        ...
        dataset,
        batch_size,
        sort_key=lambda x, index_1, index_2: max(len(x[index_1]), len(x[index_2])),
        input_key: Union[str, int] = 0,
        label_key: Union[str, int] = 1,
    ):
        self.dataset = dataset
        ...
        indices = np.argsort(
            [
                self.sort_key(self.dataset[i], self.index_1, self.index_2)
...
def collate_fn_trunc(batch, max_len, eos_token_id, pad_token_id):
    ...
    padded_input_seqs[:, -1:][
        (padded_input_seqs[:, -1:] != eos_token_id)
        & (padded_input_seqs[:, -1:] != pad_token_id)
    ] = eos_token_id

    padded_target_seqs[:, -1:][
        (padded_target_seqs[:, -1:] != eos_token_id)
        & (padded_target_seqs[:, -1:] != pad_token_id)
    ] = eos_token_id
